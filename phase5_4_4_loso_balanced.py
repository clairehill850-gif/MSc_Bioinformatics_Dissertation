#!/usr/bin/env python3
# What this does: Fine-tune balanced
import sys
import json
import copy
...
from phase2_1_2_transforms_Alaska import get_train_transform, get_val_transform
...
RESULT = OUT / "loso_balanced_predictions.csv"
...
# Fixed seed
        ...
        ref_subset = Subset(ref_train, pick)
        mixed = ConcatDataset([slide_ds, ref_subset])

# Balancing
        counts = tr["true_label"].value_counts()
        slide_w = [1.0 / counts[l] for l in tr["true_label"]]
        ref_w = [float(np.mean(slide_w))] * len(ref_subset)
        weights = torch.DoubleTensor(slide_w + ref_w)
        sampler = WeightedRandomSampler(weights, num_samples=len(mixed), replacement=True)
        loader = DataLoader(mixed, batch_size=BATCH, sampler=sampler,
...
# Results
    ...
    print("\n=== LEAVE-ONE-SLIDE-OUT RESULT (class-balanced sampling) ===")
...
# Accuracy
    ...
    print("\nper taxon | baseline on these grains: Betula 1/19, Populus 0/16, Salix 9/11 | unbalanced LOSO: Betula 12/19, Populus 7/16, Salix 2/11")
